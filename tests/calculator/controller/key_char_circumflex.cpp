...
TEST(controller, key_char_circumflex_too_few_elements) {
  tmodel model;
...
TEST(controller, key_char_circumflex_stack_input) {
  ...
  EXPECT_EQ(model.stack().strings(), std::vector<std::string>{"0b1101"});
  EXPECT_TRUE(model.input_get().empty());
}
...
TEST(controller, key_char_circumflex_diagnostics_cleared) {
  tmodel model;
  tcontroller controller{model};
