...
TEST(controller, math_or_too_few_elements) {
  tmodel model;
...
TEST(controller, math_or_stack_input) {
  tmodel model;
  tcontroller controller{model};
...
TEST(controller, math_or_stack_stack) {
  tmodel model;
  tcontroller controller{model};
  model.stack_push(0b1101);
  model.stack_push(2);
...
TEST(controller, math_or_diagnostics_cleared) {
  tmodel model;
  tcontroller controller{model};
...
TEST(controller, math_or_input_invalid) {
  tmodel model;
  tcontroller controller{model};
  model.stack_push(42);
