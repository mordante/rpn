...
TEST(controller, math_and_too_few_elements) {
  tmodel model;
...
TEST(controller, math_and_stack_input) {
  tmodel model;
  tcontroller controller{model};
  ...
  EXPECT_EQ(model.stack_size(), 1);
  EXPECT_EQ(model.stack_pop().get(), 2);
  EXPECT_TRUE(model.input_get().empty());
}

TEST(controller, math_and_stack_stack) {
  tmodel model;
  tcontroller controller{model};
  model.stack_push(0b1111);
  model.stack_push(2);
...
TEST(controller, math_and_diagnostics_cleared) {
  tmodel model;
  tcontroller controller{model};
...
TEST(controller, math_and_input_invalid) {
  tmodel model;
  tcontroller controller{model};
  model.stack_push(42);
