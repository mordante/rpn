...
TEST(controller, key_char_plus_too_few_elements) {
  tmodel model;
...
TEST(controller, key_char_plus_stack_input) {
  tmodel model;
  tcontroller controller{model};
  handle_input(controller, model, "42");
  ...
  EXPECT_EQ(model.stack().strings(), std::vector<std::string>{"84"});
  EXPECT_TRUE(model.input_get().empty());
}

TEST(controller, key_char_plus_stack_stack) {
  tmodel model;
  tcontroller controller{model};
  handle_input(controller, model, "42");
  handle_input(controller, model, "42");
...
TEST(controller, key_char_plus_diagnostics_cleared) {
  tmodel model;
  tcontroller controller{model};
...
TEST(controller, key_char_plus_input_invalid) {
  tmodel model;
  tcontroller controller{model};
  handle_input(controller, model, "42");
