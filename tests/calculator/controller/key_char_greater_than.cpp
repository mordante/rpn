...
TEST(controller, key_char_greater_than_too_few_elements) {
  tmodel model;
...
TEST(controller, key_char_greater_than_stack_input) {
  tmodel model;
  ...
  EXPECT_EQ(model.stack(), std::vector<std::string>{"0b1010"});
  EXPECT_TRUE(model.input_get().empty());
}

TEST(controller, key_char_greater_than_stack_stack) {
  tmodel model;
  model.base_set(tbase::binary);
  tcontroller controller{model};
  handle_input(controller, model, "0b101010");
...
TEST(controller, key_char_greater_than_diagnostics_cleared) {
  tmodel model;
  model.base_set(tbase::binary);
  tcontroller controller{model};
...
TEST(controller, key_char_greater_than_input_invalid) {
  tmodel model;
  tcontroller controller{model};
  handle_input(controller, model, "42");
...
TEST(controller, key_char_greater_than_shift_too_small) {
  tmodel model;
  tcontroller controller{model};
  handle_input(controller, model, "42");
...
TEST(controller, key_char_greater_than_shift_too_large) {
  tmodel model;
  tcontroller controller{model};
  handle_input(controller, model, "42");
