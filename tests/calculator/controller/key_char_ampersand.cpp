...
TEST(controller, key_char_ampersand_too_few_elements) {
  tmodel model;
...
TEST(controller, key_char_ampersand_stack_input) {
  tmodel model;
  tcontroller controller{model};
  handle_input(controller, model, "0b1111");
  handle_input(controller, model, "2");

  controller.handle_keyboard_input(tmodifiers::none, '&');
  EXPECT_TRUE(model.diagnostics_get().empty());
  EXPECT_EQ(model.strings(), (std::vector<std::string>{{"2"}}));
  EXPECT_TRUE(model.input_get().empty());
}
...
TEST(controller, key_char_ampersand_diagnostics_cleared) {
  tmodel model;
  tcontroller controller{model};
